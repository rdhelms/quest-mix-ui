...
export class ObjectEditorComponent implements OnInit, OnDestroy, AfterViewInit {

    @Input() canvasSize!: number;
    @Input() objectId?: number;

    @ViewChild('objectEditorCanvas') canvasRef?: ElementRef;
    @ViewChild('objectPreviewCanvas') previewRef?: ElementRef;
    ...
    async ngOnInit() {
        // if (this.objectId !== undefined) {
            const loadedObject = await this.objectService.getObject(/*this.objectId*/);
            if (loadedObject) {
                this.name = loadedObject.name || '';
                this.frames = loadedObject.frames;
            }
        // }
    ...
    save(evt?: MouseEvent) {
        if (this.currentFrame) {
            ...
            this.objectService.saveObject(object).then(() => {
    ...
    copyFrame() {
        this.objectService.copyFrame(this.currentFrame);
    }

    pasteFrame() {
        const frameToPaste = this.objectService.pasteFrame();
        if (frameToPaste) {
            const currentIndex = this.frames.indexOf(this.currentFrame);
